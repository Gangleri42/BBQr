#!/usr/bin/env python3
"""Generate bbqr/testdata/vectors.json from the Python reference.

Development tool only; not run in CI. The reference is python/ in this
repository (BBQR_REF points elsewhere) and needs pyqrcode:

    python3 -m venv venv && venv/bin/pip install pyqrcode
    venv/bin/python gen_vectors.py > vectors.json
"""
import json
import os
import random
...
noise = random.Random(42).randbytes(2048)
add("random-32", noise[:32], "B")
add("random-91", noise[:91], "B")   # 146 chars, past version 5's 144: one version 6 part
add("random-92", noise[:92], "B")   # 148 chars, one version 6 part
add("random-2048", noise, "B")      # multiple parts at any version
