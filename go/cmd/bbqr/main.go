...
// machine. A holder of one derived share can verify a guess at the
// data, so the profile is for data whose entropy defeats guessing
// even by someone who knows part of it, such as a wallet descriptor.
// A seed is better served by the default.
package main
...
the data and k, at the price that one share verifies a guess at the
data; use it for data nobody can guess even knowing part of it, such
as a descriptor. The default is the safe choice for a seed.`)
}
