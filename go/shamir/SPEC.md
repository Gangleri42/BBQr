...
A threshold extension to [BBQr](https://github.com/coinkite/BBQr).
Any data is compressed, sealed with its file type and an integrity
digest, split into a k-of-n Shamir scheme, and each share is carried
by its own BBQr series of file type `M`. Under the randomized
generator profile, fewer than k shares reveal nothing about the data
beyond the threshold, the tag and the exact sealed length (see
Security), in the information-theoretic sense. The derived profile
(section 3a) trades that guarantee for shares reproducible from the
data; it is for data whose entropy defeats guessing. BBQr's framing is unchanged: transport,
...
The file type `M`, as in m-of-n, is reserved upstream, and this
...
(`github.com/Gangleri42/BBQr/go/shamir`) is the reference implementation, and
...
type byte and digest. Generators emit the envelope as one BBQr series of file type `M` in
base 32 encoding (`2`); shares are uniformly random, so the `Z`
encoding could never shrink them. Receivers join any BBQr encoding, as
vanilla BBQr does. Parts are sized as the BBQr
...
family of codes. A 32-byte secret split 2-of-4 fits a single
...
combination cures it. The digest covers the compressed payload, so only
...
1. Combine the first k shares in the order they were collected and
   verify the digest. If it verifies,
...
1716 exceeds the cap). Above that the reference
...
half of the held shares agree with is checked against the first k of
its dissenters in collection order (with fewer than k dissenters the
reading stands), and the larger agreement wins, equal agreement
...
reference compressor's output (`internal/deflate`, frozen by its own
golden test); a generator that must reproduce a set byte for byte must
emit that bitstream or take the payload from a conforming generator. Compression is validated by
...
TestVectors -update`. Run the checker from `go/`:
