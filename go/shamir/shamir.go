...
// descriptor. See [SHAMIR.md] for the format and the security argument.
//
// [BBQr]: https://github.com/coinkite/BBQr
// [SHAMIR.md]: https://github.com/Gangleri42/BBQr/blob/master/SHAMIR.md
package shamir
...
// independent of s unless k shares are combined, so rand must be a
// cryptographic random source.
// Coefficient bytes are hedged (see SHAMIR.md): XORed with an
...
// fewer shares than the threshold yields a wrong result with no error,
// which is why transports (such as the share envelopes of this
// package) must add integrity. All shares must have the same
...
// the exact derivations are in SHAMIR.md): prk = HMAC-SHA256(label,
