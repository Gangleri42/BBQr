...
// reconstruction; it is not a MAC (see SHAMIR.md).
const (
...
// SHAMIR.md, and encodes each share as its own type M BBQr series in
...
// cryptographic random source. Privacy
...
// stream reproduces a split exactly, which is how the randomized test
// vectors in SHAMIR.md are defined.
//
...
// SHAMIR.md: the same validation, compression, sealing, envelope, BBQr
...
// deterministic function of the sealed content, so a holder of one
// share can verify a guessed data value against it. The profile is
// for data whose min-entropy defeats guessing, such as a wallet
// descriptor whose extended public keys each carry a 256-bit chain
// code. A generated mnemonic has the bits, but a single share turns
// any partial leak of it into a confirmed brute force, and two splits
// of one seed carry the same tag and shares; SplitData is unaffected
// by either, so seeds and anything a person could enumerate belong
// there.
...
// then the first threshold with each member swapped for each spare in
// turn (k·(held-k) attempts), then the disjoint windows of threshold
// positions after the first. A reading that at most half of the held
// shares agree with is checked against one combination of its
// dissenters (see outvote). That attribution is exact for one corrupt
// share and for a cancelling pair outnumbered by the clean shares,
// and unverified beyond. SHAMIR.md states the limit.
