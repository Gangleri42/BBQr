...
status 1 on any FAIL. Standard library only.
"""
import base64
import binascii
import hashlib
...
def size_parts(nbytes):
    ...
    for version in range(MIN_VERSION, MAX_VERSION + 1):
        per_part = (ALNUM_CAPACITY[version] - HEADER_LEN) // 8 * 5
        candidates.append((-(-nbytes // per_part), version, per_part))
...
def check(v):
    ...
    for i, (env, share) in enumerate(zip(envelopes, v["shares"]), 1):
        ...
        if parts != share["parts"]:
            ...
            p = first_diff(parts, share["parts"])
