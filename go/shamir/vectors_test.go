...
// The generator profiles of SHAMIR.md, as the vectors name them.
const (
...
//
// python/tests/test_shamir.py runs the same generator and receiver
// classes against the Python implementation, reading the repository's
// copy, test_data/shamir_vectors.json; -update writes both copies.
...
// descriptorCBORHex is the crypto-output CBOR (BCR-2020-010) of
// descriptorText, the C-typed input of the descriptor-cbor-2-of-3
...
// that this package needs no CBOR or descriptor dependency.
...
// sealedDigest restates section 2 of SHAMIR.md: SHA-256(k ‖ T ‖ payload)[0:4].
...
func TestVectors(t *testing.T) {
	if *update {
		...
		// The module's copy and the repository's, which the Python
		// tests read.
		for _, name := range []string{
			"testdata/vectors.json",
			filepath.Join("..", "..", "test_data", "shamir_vectors.json"),
		} {
	...
	// for share (SHAMIR.md, section 3a): every pair of derived vectors
